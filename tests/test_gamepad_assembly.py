...
class GamepadAssemblyTests(unittest.TestCase):
    ...
    def test_build_drag_moves_for_origin_cell(self):
        ...
        self.assertEqual(StickMove(0.0, -0.5, 0.02), moves[0])
    ...
    def test_hold_move_release_sequence(self, perf_counter, _sleep):
        perf_counter.side_effect = [0.0, 0.0, 0.02, 0.02, 0.03, 0.03, 0.04, 0.04]
        ...
        with patch.dict(
            ...
            controller.move_left_stick(0.2, -0.3, 0.02)
            self.assertEqual((0.2, -0.3), fake_gamepad.left_stick)
            ...
            self.assertGreaterEqual(fake_gamepad.updates, 3)
